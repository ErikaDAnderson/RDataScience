...
purrr is more elegant, but my for loop gives the results of each vector,not just all columns. Can you think of why every would be more useful than getting all results from list?
...
output2 <- vector("logical", length(y))

# use for loop instead
for (i in seq_along(y)) {output[[i]] <- is.numeric(y[[i]])} 
output
for (i in seq_along(y)) {output2[[i]] <- is.integer(y[[i]])} 
output2
...
# column summary for numeric columns only
...
for (i in seq_along(df)) {
  ...
  output[[i]] <- if_else(is.numeric(df[[i]]) == TRUE, mean(df[[i]]), NA_real_)
}
output
