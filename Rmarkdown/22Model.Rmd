...
optimMean <- optim(c(4,2), measure_distance2, data = sim1)
optimMean

# compare to root mean squared deviation
optimRMSD <- optim(c(4, 2), measure_distance, data = sim1)
optimRMSD
...
ggplot(aes(x,y), data = sim1) +
  ...
  geom_abline(slope = optimMean$par[2], intercept = optimMean$par[1], color = "red") +
  geom_abline(slope = optimRMSD$par[2], intercept = optimRMSD$par[1], color = "blue") +
  theme_bw()

```

It looks like mean distance is similar to RMSD.
...
ggplot() +
  ...
  geom_smooth(aes(x, y), method = "loess",  data = sim1) +
...
From the ggplot package to add a reference line to the graph. It can be used to mark the zero mark for residuals to see if there are any patterns left in the data.
